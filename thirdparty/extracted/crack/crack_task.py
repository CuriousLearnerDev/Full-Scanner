...
class CrackTask:
    ...
    def run(self, id, url,result=None):
        self.id = id
        self.url = url
        self.result=result
        print("")
        Log.init_log_id(id)
        Log.Info(f"[*] 开始: {url}",self.result)
        try:
            self.parser = Parser(self.url,self.result)
            if not self.parser.run(self.result):
                return
            ...
            if not username and not password:
                ...
                if sqlin_dict_enable:
                    Log.Info(f"[*] {url} 启动万能密码爆破模块",self.result)
                    sqlin_user_dict, sqlin_pass_dict = gen_sqlin_dict()
                    username, password = self.crack_task(sqlin_user_dict, sqlin_pass_dict)

            if username and password:
                Log.Info(f"[*] 正在重新检查... {url} {username} {password}",self.result)
                recheck_flag = self.recheck(username, password)
                if recheck_flag:
                    Log.Success(f"[+] 成功: {url}  {username}/{password}",self.result)
                    return
                else:
                    Log.Info(f"[-] 重新检查失败: {url}  {username}/{password}",self.result)
            Log.Error("[-] 失败的: " + url,self.result)
        except Exception as e:
            Log.Error(f"{str(e)}",self.result)
    ...
    def crack_task(self, username_dict, password_dict):
        ...
        for username in username_dict:
            for password in password_dict:
                ...
                password = password.replace('{user}', username)
                num = num + 1
                Log.Info(f"[*] {self.url} 进度: ({num}/{dic_all}) checking: {username} {password}",self.result)
