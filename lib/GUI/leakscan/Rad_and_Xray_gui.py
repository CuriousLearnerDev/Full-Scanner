...
def Rad_and_Xray_invoke(self):
    ...
    # 设置文本框控件
    self.Rad_and_Xray_result = ttk.Text(Backupfilescan_top,
                                        yscrollcommand=Backupfilescan_sbar.set,  # 调用滚动条
                                        undo=True)  # 开启删除内容

    # 在主窗口内显示
    self.Rad_and_Xray_result.place(relwidth=0.988,
                                   relheight=1.0)

    # font1 = tk.font.Font(family='微软雅黑',size=24) # 设置字体
    font = tk.font.Font()
    self.Rad_and_Xray_result.config(font=font, foreground='#008B00')  # 颜色

    Backupfilescan_sbar.config(command=self.Rad_and_Xray_result.yview)  # 设置鼠标可以

    for i in range(30):
        self.Rad_and_Xray_result.insert(tk.END,
    ...
    try:
        self.Rad_and_Xray_result.edit_undo()  # 清空内容
    except:
        pass

    from leakscan.rad_xray_py import rad_xray
    # 定义线程 预防卡
    t = threading.Thread(target=rad_xray.main_gui, args=(self.Rad_Xray_dingtalk_batch.get(),
                                                         self.Rad_and_Xray_result))
    # 启动线程
    t.start()
...
def Rad_and_Xray(self,top):
    ...
    # 显示线
    fofa_right = ttk.LabelFrame(top, text="批量扫描Rad联动Xray", bootstyle="success")
    ...
    label_1 = ttk.Button(fofa_right,
                         ...
                         bootstyle="success",
                         width=13,
                         command=self.Rad_and_Xray_invoke)
