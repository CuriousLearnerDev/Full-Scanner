...
def ssh_scanning(self):
    ...
    # 设置文本框控件
    self.ftp_result = ttk.Text(ssh_top,
    ...
    t = threading.Thread(target=ssh.gui, args=(self.ssh_host.get(),
                                               self.ssh_port.get(),
                                               self.ssh_thread.get(),
                                               self.ssh_user.get(),
                                               self.ssh_passwd.get(),
...
def ssh(self,top):
    ...
    ssh_user = ttk.Entry(fofa_right,
                         textvariable=self.ssh_user,
                         bootstyle="success",
                         width=30)
    ssh_user.grid(row=3,
                  column=2,
                  pady=10,
                  sticky=tk.W)
    ssh_user.insert(0, config.Specifyablastdictionary['ssh']['admin'])
    # 选择文件
    ...
    self.ssh_passwd = tk.StringVar()
    ssh_passwd = ttk.Entry(fofa_right,
                           textvariable=self.ssh_passwd,
                           bootstyle="success",
                           width=30)

    ssh_passwd.grid(row=4,
                    column=2,
                    pady=10,
                    sticky=tk.W)
    ssh_passwd.insert(0, config.Specifyablastdictionary['ssh']['passwd'])

    # 选择文件
    passwd_file = tk.Button(fofa_right,
                            text='选择文件',
                            relief=tk.RAISED,
                            command=self.ssh_passwd_many)
    ...
    # 端口
    # 生成动态字符串
    self.ssh_port = tk.StringVar()
    ssh_port = ttk.Entry(fofa_right, textvariable=self.ssh_port, bootstyle="success", width=10)
    ssh_port.grid(row=3, column=5, sticky=tk.W)
    ssh_port.insert(0, '22')
    ...
    # 端口
    # 生成动态字符串
    self.ssh_thread = tk.StringVar()
    ssh_thread = ttk.Entry(fofa_right, textvariable=self.ssh_thread, bootstyle="success", width=10)
    ssh_thread.grid(row=4, column=5, sticky=tk.W)
    ssh_thread.insert(0, '1')
