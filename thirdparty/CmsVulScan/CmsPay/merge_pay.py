...
import os
import sys
import thirdparty.CmsVulScan.modules.function as fun
# sys.path.append('../')
import importlib
geshu = 0
def generate():
    ...
    try:
        NewPay = open("thirdparty/CmsVulScan/CmsPay/payload.txt", "w", encoding="utf-8")
        for i in os.listdir("Dict"):
            file = open("thirdparty/CmsVulScan/Dict/" + i + "/payload.txt", "r", encoding="utf-8")
...
def dete():
    ...
    print(fun.times()+fun.col("[*] 检查payload文件是否存在","yellow"))
    if not os.path.exists("thirdparty/CmsVulScan/CmsPay/payload.txt"):
...
def Dr_list():
    jihe = set()
    file = open("thirdparty/CmsVulScan/CmsPay/payload.txt","r",encoding="utf-8")
    lis = file.readlines()
    file.close()
    for i in lis:
        i = i.replace("\n","")
