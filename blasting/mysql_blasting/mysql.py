...
def try_to_log_in(Thread,host,port):
    ...
    while not Thread.empty():
        ...
        if pause == True:# 破解成功用于暂程和禁止输出的
            ...
            try:
                pymysql.connect(host=host, user=user_passwd[0], password=user_passwd[1])
                schedule += 1
                if GUT_Sure == 1:
...
def Interface(args):
    ...
    if user == None: # 默认用户字典
        ...
        user = config.Specifyablastdictionary['mysql']['user']
    ...
    if document == None: # 默认密码字典
        ...
        document = config.Specifyablastdictionary['mysql']['passwd']
